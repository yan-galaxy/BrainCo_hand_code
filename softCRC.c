...
static uint32_t CRCtable_CRC32[256] = {
...
static uint16_t CRCtable_CRC16_CCITT[256] = {
...
/**
 * Due to pointer conversion, when pData is not align to 0x04, pool performance
 */
uint32_t softCRC_CRC32(const void *pData, size_t Length, uint32_t seed, uint32_t final)
{
    const uint32_t *data = (const uint32_t *)pData;
    uint32_t nReg = seed, nTemp = 0;
    uint16_t i, n;
    for (n = 0; n < Length; n++)
    {
        nReg ^= data[n];
        for (i = 0; i < 4; i++)
        {
            nTemp = CRCtable_CRC32[(uint8_t)((nReg >> 24) & 0xff)]; // look up table from the MSByte of nReg
            nReg <<= 8;                                             // left shift one byte, fill zero to LSByte
            nReg ^= nTemp;                                          // XOR with previous table result.
        }
    }
    return nReg ^ final;
}
...
uint16_t softCRC_CRC16_CCITT(const void *pData, size_t length, uint16_t seed, uint16_t final)
{
    ...
    uint16_t crc = seed, temp;
    size_t count;
    for (count = 0; count < length; ++count)
    {
        temp = (*data++ ^ (crc >> 8)) & 0xff;
        crc = CRCtable_CRC16_CCITT[temp] ^ (crc << 8);
    }
